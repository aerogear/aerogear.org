...
Jenkins instance to connect with the macOS nodes. Below is the steps involved
...
no guarenteed support for concurrent builds. This value should not be changed
unless you are certain all apps will be built with the same signing
credentials.
|buildfarm_node_mode
