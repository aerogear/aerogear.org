...
This brings you the a page that where you can configure details about your account. The first option allows you to provide additional information like your name or your email address:
...
The Dashboard also has a +Warning+ and a +Most Active+ section. The +Warning+ aera informs you if a problem occurred, while sending out the Push Notifications to the 3rd party Services. Clicking on an entry in that list provides you more details about the potential failure.

The +Most Active+ section shows a recent list of PushApplications did submit Push Notification requests. Clicking on a PushApplication presents a list of all Push Notifications that have been sent out (in the last 30 days):
