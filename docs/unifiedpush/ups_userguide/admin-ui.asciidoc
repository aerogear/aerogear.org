...
The +developer+ accounts have full access to **UnifiedPush AdminUI**. This includes full management of their own PushApplications and Variants as well as sending Push Notification from the UnifiedPush AdminUI.

The +admin+ has full access to **UnifiedPush AdminUI** _and_ the **Keycloak Admin Console**. On the UnifiedPush AdminUI the admin has full control of all users PushApplications and Variants. On the Keycloak Admin Console the admin can perform some administrative configurations around the UnifiedPush Server and its users.

IMPORTANT: By default the +developer+ account is disabled and needs to be explicitly enabled by the +admin+ inside of the link:../server-administration[Keycloak Admin Console]!
