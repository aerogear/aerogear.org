...
On top of OAuth2 authorization grant flow, you can use *login* which will behaves the same way as *requestAccess*:
1. check if access token is valid, if already there just run callback method
2. if not valid and refresh token prsent, go and fetch new access token
3. in none of the above triggers a prop-up to authenticate and grant access. Additional scope is required to retrieve user profile information.
