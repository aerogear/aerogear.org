...
Download the server from the project's link:https://github.com/aerogear/aerogear-simplepush-server[github page]. For the needs of the project, a custom Netty SockJS adapter implementation was written, but since it's not yet integrated in the Netty mainline (planned in an upcoming release) we need to install it manually for now. Simple issue the following command to install it in your local maven repository:
