...
A Store represents an abstraction layer for a storage system. Currently implemented storage systems are an link:/docs/specs/aerogear-ios/Classes/AGMemoryStorage.html[in-memory], link:/docs/specs/aerogear-ios/Classes/AGPropertyListStorage.html[property list] and a link:/docs/specs/aerogear-ios/Classes/AGSQLiteStorage.html[SQLite] storage (note that encrypted variants of these stores are also available, check the link:/docs/guides/aerogear-ios/Cryptography/[Cryptography support] section below).
