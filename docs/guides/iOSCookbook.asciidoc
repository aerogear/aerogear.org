...
Since 1.3.0 release, iOS AeroGear provides _Symmetric encryption API_ and enhances its Core API with _Encrypted Stores_.
If you need to encrypt only a few fields in your data, use encryption API directly, if all your data are sensitive, AeroGear provides a Encrypted Storage which transparently let you access your encrypted data. Let's delve into how to use encryption to secure your enterprise app.

=== Symmetric encryption API

Symmetric encryption (also called private key encryption) is when the same key is used to encrypt and decrypt data. It is best defined with 4 basic concepts:
...
NOTE: To be able to decrypt, you need to store the encryption key and the randomly generated IV.
You can store the sensitive information in KeyChain for example.
...
Current available implementation are:
...
1. and link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGPropertyListStorage.html[encrypted property list] storage implementation for a permanent storage.

link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGSQLiteStorage.html[Encrypted SQLite] will come in next release.

In order to keep your password safe, you can use *AGEncryptionService* which takes care of storing the password/passphrase
required for encryption. Those information should be kept secret and are needed in order to decrypt the encrypted data.
AGEncryptionService stores your secret.

You can initialize an encrypted store with an _alias_ and a _password_. Three possible scenarios can occur:

1. If alias/password are not present in AGEncryptionService,
they will be stored. AGEncryptionService will be made available for encrypted store initialization.
1. If the alias already exists and the password is correct, AGEncryptionService is available.
1. If the alias exists but password is wrong, AGEncryptionService is not initialized. It is up to the application developer
 to deal with the wrong credentials use case.
...
----
    ...
    [config setAlias:@"MyUser"];
    [config setPassword:@"MyUserPassword"];
    ...
    if (!encService) {
        // TODO: deal with wrong credentials
    }
...
This application shows you how using encrypted storag,e you can store in a central point all your passwords.
