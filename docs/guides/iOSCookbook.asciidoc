...
If your storage information need to be encrypted, AeroGear provides convenient and transparent encrypted storage support. You can use encrypted store with the same API than plain store. 


Currently available is link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGMemoryStorage.html[encrypted memory], link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGPropertyListStorage.html[encrypted property list] storage implementation. link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGSQLiteStorage.html[Encrypted SQLite] will come in next release.
...
Enter your passphrase, all your existing passwords will be decrypted. AeroGear Crypto Password secures your data using AeroGear-Crypto library underneath powered by CBC symmetric encryption.
