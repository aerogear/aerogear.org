...
Since 1.3.0 release, iOS AeroGear provides _Symmetric encryption API_ and enhances its Core API with _Encrypted Stores_. Let's delve into how to use encryption to secure your enterprise app.
...
- *IV (Initialization Vector)* s a random value that is used to encrypt data. Encryption algorithms usually work on fixed-size blocks, IV defines the first encrypted block.
...
For random generation of key, use link:http://aerogear.org/docs/specs/aerogear-crypto-ios/Classes/AGRandomGenerator.html[AGRandomGenerator]. By default, AGRandomGenerator generates 16 bytes key, but you can also specify the length if you wish.
...
NOTE: To be able to decrypt you need to store the encryption key and the ramdonly generated IV. You can store the sensitive information in KeyCahin for example.
...
Current available implementation are: link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGMemoryStorage.html[encrypted memory store] which allows you to work in-memory with sensitive data and link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGPropertyListStorage.html[encrypted property list] storage implementation. 
link:http://aerogear.org/docs/specs/aerogear-ios/Classes/AGSQLiteStorage.html[Encrypted SQLite] will come in next release.

Before

To create a DataManager with encrypted permanent storage, you simply have to specify in the config the type *ENCRYPTED_PLIST*, and here you go you've got your property list encrypted storage available!
[source,c]
----
TODO
...
// create the datamanager
AGDataManager* dm = [AGDataManager manager];
// add a new encrypted store object:
id<AGStore> encryptedStore = [dm store:^(id<AGStoreConfig> config) {
   [config setName:@"tasks"];
...
If you want to see it all together, give a try to ```AeroGear Crypto Password``` app. Using encrypted storage you can store in a central point all your passwords. Intial load of application will prompt you to enter you password, store it in keychain so that when you later login.  passphrase, all your existing passwords will be decrypted. 
