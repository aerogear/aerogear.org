...
* Passing the config parameters directly in th JavaScript code :
...
* Using an external configuration file named _push-config.json_ and placed inside the _www_ folder :
...
platform you can simplfy the config (or your push-config.json file) above by:
