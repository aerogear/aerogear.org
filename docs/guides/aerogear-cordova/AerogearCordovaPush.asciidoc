...
To change the value to `production` you have to provide your own Push.entitlements file. You can pick the link:https://github.com/aerogear/aerogear-cordova-push/blob/master/src/ios/Push.entitlements[example file], change the value to `production` and put it inside the www folder. It's values will be copied to the project when `cordova prepare ios` is run.
