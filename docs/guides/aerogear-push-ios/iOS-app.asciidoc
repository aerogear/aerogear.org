...
Afterwards, please clone this link:https://github.com/aerogear/aerogear-push-ios-demo[Sample Application]. It uses our _early_ SDK to register your _Mobile Variant Instance_ (the application on the device) with the iOS Variant you registered earlier!
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    ...
    [registration registerWithClientInfo:^(id<AGClientDeviceInformation> clientInfo) {   // [2]
        ...
        [clientInfo setMobileVariantID:@"402880e63ea239b9013ea23be9dc0004"];  // [3]
        
        // apply the token, to identify THIS device
        [clientInfo setDeviceToken:deviceToken];  // [4]

        // --optional config--
        // set some 'useful' hardware information params
        UIDevice *currentDevice = [UIDevice currentDevice];
        
        [clientInfo setOperatingSystem:[currentDevice systemName]];  // [5]
        [clientInfo setOsVersion:[currentDevice systemVersion]];     // [6]
        [clientInfo setDeviceType: [currentDevice model]];           // [7]
...
Next, in [2] we call _registerWithClientInfo_ passing a configuration block that sets up the parameters needed to register this app with the _AeroGear Push Server_. The required parameters that need to be present are the _MobileVariantID_ [3] (which we received it in the previous chapter when we were registering the _iOS Variant_ with the _Push Application_) and the Device Token [4]. It is valid to specify a bit more data on the _clientInfo_ and we do here by setting metadata such the operating system [5], its version [6] and the device type [6].
