...
_Note:_ APNs interactive notification adds +category+ to its payload to associate a group of actions to a category of notification. As UPS already uses +categories+ payload for a different purpose, 'action-category' payload is the property used for linking actions to an interactive notification category.
