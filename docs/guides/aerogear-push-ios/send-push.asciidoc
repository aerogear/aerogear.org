...
A bit earlier you received a _pushApplicationID_ and a _masterSecret_ after the successful registration of the _Push Application_. These ID's  are used for sending a _broadcast_ message to all _registered_ devices (_Mobile Variant instances_). Now, on a command line issue the following _CURL_ command:
