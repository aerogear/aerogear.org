...
*HSTS* (HTTP Strict Transport Security) is a security policy mechanism which enforce users to interact with SSL connections only, preventing attacks like man-in-the-middle.
...
Basically, browsers that support this feature, are able to remember the fact that a particular website requires HTTPS only, the next visit the respective site the agent will remember about this option on HTTPS only. 
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		git@github.com:abstractj/aerogear-controller-demo.git
